...
public class FlightCriteriaValidator implements Validator {
	...
	public void validate(Object target, Errors errors) {
		...
		DateTimeFormatter dateFormatter = DateTimeFormatter
				.ofPattern(configuration.getDatePattern());
		...
		if (flightCriteria.getFrom() == null || flightCriteria.getDateTo() == null
			...
			// Valid airport code
			if (!configuration.getValidAirportCodes().contains(flightCriteria.getFrom())) {
			...
			if (!isValidDatePattern(flightCriteria.getDateTo())) {
				...
				errors.rejectValue("dateTo", ErrorMessage.DATE_IN_PAST.getMessage());
					...
					&& LocalDate.parse(flightCriteria.getDateTo(), dateFormatter).isBefore(
							LocalDate.parse(flightCriteria.getDateFrom(), dateFormatter))) {
	...
	/**
	 * Checks if a String representation of a date can be converted to a
	 * {@link LocalDate} and if it has the correct format
	 * 
	...
	private boolean isValidDatePattern(String date) {
		...
		DateTimeFormatter formatter = DateTimeFormatter.ofPattern(configuration.getDatePattern());
